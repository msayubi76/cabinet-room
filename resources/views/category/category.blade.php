                                    ...
                                    <div class="row d-flex">
                                        ...
                                        <div class="col-lg-6">
                                            <div class="float-right d-print-none">
                                                @can('category.create')
                                                    <a href="javascript:;" data-target="#create_modal" id="create_category_btn"    data-toggle="modal"  class="btn btn-primary float-right">Create Category</a>
                                                @endcan
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="datatable-buttons" class="table table-striped table-bordered w-100 user_table">
                                            ...
                                            <tr>
                                                ...
                                                @if (Auth::user()->hasAnyPermission(['category.update', 'category.delete']) || Auth::user()->hasRole('Super Admin'))
                                                    <th>Action</th>
                                                @endif
                                            ...
                                            @foreach ($category as $value)
                                                <tr class="gradeX">
                                                    ...
                                                    <td >
                                                        @if ($value->image)
                                                            <a href="{{url('site_images/categories/'.$value->image)}}" class="image_link" title="View" target="_blank">
                                                                <img src="{{url('site_images/categories/'.$value->image)}}" class="category_image" alt="{{$value->name}}" width="50px" height="50px">
                                                            </a>
                                                        @endif 
                                                    </td>
                                                    @if (Auth::user()->hasAnyPermission(['category.update', 'category.delete']) || Auth::user()->hasRole('Super Admin'))
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form   class="custom-validation " id="create_form" enctype="multipart/form-data" action="{{url('category')}}" method="post"  >
                                           @csrf
                                            <div class="row">
                                                <div class="col-md-12">
      ...
      //  var form = $("#create_form").serialize();
        ...
        $.ajax({
            ...
            if (response.status) {
                ...
                $(clonedRow).find('.category_image').html(response.category.category_sign);
              

                if( response.category.image != "undefined" ){
                    $(clonedRow).find('.image_link').attr('href', "{{url('site_images/categories')}}/"+response.category.image);
                    $(clonedRow).find('.category_image').attr('src', "{{url('site_images/categories')}}/"+response.category.image);
                }
    ...
    $(document).on('click', '.submit_edit_form', function(e) {
        ...
        $.ajax({
            ...
            if (response.status == true && response.success == true) {
                ...
                if(response.category.image != "" || response.category.image != undefined){
                   $(editing_row).find('.image_link').attr('href', "{{url('site_images/categories')}}/"+response.category.image);
                   $(editing_row).find('.category_image').attr('src', "{{url('site_images/categories')}}/"+response.category.image);
                }
